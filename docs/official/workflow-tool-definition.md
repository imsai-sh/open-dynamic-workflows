<!--
═══════════════════════════════════════════════════════════════════════════════
官方定义 · OFFICIAL DEFINITION (verbatim) — for review / fidelity reference
───────────────────────────────────────────────────────────────────────────────
来源 Source:  Claude Code 内置 `Workflow` 工具的工具描述（the verbatim description of
              Claude Code's built-in `Workflow` tool）, Claude Code v2.1.156.
检索 Retrieved: 2026-05-30.
说明 Note:    这是"如何编写 / 运行动态 workflow"的【权威契约原文】——逐字、未删改，仅作存档对照。
              注意：本文件【含 budget】（官方原文如此）；但**本运行时 v1 故意不支持 budget**（精简）。
              面向使用者的撰写引导以 skills/open-dynamic-workflows/SKILL.md 为准（已去掉 budget）。
              配套阅读 workflows.md（公开文档）与 blog-introducing-dynamic-workflows.md（发布博客）。
              This is the verbatim official text, kept for reference only. NOTE: it INCLUDES
              budget, but the v1 runtime intentionally does NOT support budget (trimmed). The
              user-facing authoring guide is skills/open-dynamic-workflows/SKILL.md. Companion: workflows.md + blog.
═══════════════════════════════════════════════════════════════════════════════
-->
...
Scripts are plain JavaScript, NOT TypeScript — type annotations (`: string[]`), interfaces, and generics fail to parse. The script body runs in an async context — use await directly. Standard JS built-ins (JSON, Math, Array, etc.) are available — EXCEPT `Date.now()`/`Math.random()`/argless `new Date()`, which throw (they would break resume). Stamp results after the workflow returns, or pass timestamps in via `args`, and for randomness vary the agent prompt/label by index. No filesystem or Node.js API access.
