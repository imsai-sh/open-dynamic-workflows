...
因为 `executor` 是 **per-node** 的,同一段脚本可以混用不同 CLI——例如让一个 CLI 起稿、另一个
...
- **每个 `agent()` 都要带 `executor`**:没有默认值。缺失或未知的 executor 名(不在 host 注册表里)
  会让整个 run 报清晰错误。
...
把脚本存为项目里的 **`.odw/<name>/script.js`**——每个 workflow 一个自包含目录。若运行时
CLI 还没装,先装(它提供 `odw` 命令,别名 `open-dynamic-workflows`):

```bash
npm install -g open-dynamic-workflows      # 提供 `odw` 命令
```

然后按名字(解析 `.odw/<name>/script.js`)或按显式路径跑:

```bash
odw run --name audit --args '{"dir":"src"}'        # 跑 .odw/audit/script.js
odw run path/to/script.js [--args '<json>'] [--model <id>] [--resume <runId>]
```

每次运行的产物(源码快照、journal、events、各 agent 的 trace)落在该 workflow 目录下的
**`.odw/<name>/runs/<runId>/`**——把 `.odw/*/runs/` 加进 `.gitignore`(你的
`.odw/<name>/script.js` 脚本保留可提交)。
...
- **排查失败的 agent**:失败原因会内联显示——例如 `agent failed (subtype=error_during_execution): You've hit your usage limit…`——而不只是一个空洞的 subtype。完整细节(spawn 的 argv、退出码、stderr、原始事件流)落在 `<runDir>/agents/agent-N.jsonl`。设 `ODW_DEBUG=1` 还会把每次 spawn 的 argv、退出状态、stderr 末尾实时打到 stderr。
